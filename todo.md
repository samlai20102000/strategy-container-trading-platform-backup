...
- [ ] 建立回測指標唯一規格並修正所有計算／顯示：期初與期末權益、已實現／未實現損益、總回報、峰谷最大回撤及金額、勝率、盈虧比、利潤因子、Sharpe、Calmar、交易／cycle／fill 次數、費用與終點估值
- [ ] 對每條已註冊策略建立機器可讀的回測準備度矩陣，逐一驗證配置 schema、資料需求、adapter、支援模式、時間框架、輸出能力與 fail-closed 理由
- [ ] 在回測中心提供逐策略準備度、指標口徑、資料品質及不支援能力提示；不允許把 unsupported／not-ready 策略當成可信結果執行
- [ ] 以獨立 oracle 與固定資料集補齊正常、零交易、全贏、全輸、負權益、未平倉終點、費用、重新入市與各內建策略的單元／整合／契約回歸測試
- [ ] 執行 TypeScript、完整 Vitest、正式建置、桌面／手機與 console／network 驗收；全程不觸發實盤下單、撤單、平倉或策略啟停
- [ ] 核對 TODO、保存 checkpoint 並確認新版本已自動發布，另交付計算口徑、逐策略準備度與已知限制摘要

...
### 問題 1：回測時間框架限制和超時
- [ ] 1.1 移除硬編碼的 1m 管理週期限制，支援 1m～4d 靈活配置
- [ ] 1.2 實現分段回測邏輯（超過 7 天自動分段，每段 7 天）
- [ ] 1.3 實現回測結果聚合（合併多段結果的收益、回撤、勝率等）
- [ ] 1.4 增加回測超時時限配置（適應長時間回測）
- [ ] 1.5 測試分段回測功能（1 個月、3 個月回測）

### 問題 2：訊號日誌盈虧顯示
- [ ] 2.1 檢查所有策略的訊號記錄邏輯，確保買入/平倉都記錄盈虧
- [ ] 2.2 修復 20415 七彩虹馬丁策略的訊號盈虧顯示
- [ ] 2.3 確保新增策略也遵循相同的盈虧記錄規範
- [ ] 2.4 前端訊號日誌頁面顯示所有交易的盈虧欄位
- [ ] 2.5 測試所有策略的訊號日誌盈虧顯示

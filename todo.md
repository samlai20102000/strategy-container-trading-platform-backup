...
## 2026-08-03 回測中心全面準確性與重新入市跨模組落實
- [x] 稽核目前工作樹與既有報告，區分已發布功能、未保存草稿及需要重做的項目，避免沿用錯誤結論或重複實作
- [x] 建立 KAMA 彩虹馬丁 `reentryEnabled` 單一 canonical 讀寫／正規化 helper，禁止回測、快照、策略建立、策略列表與 runtime 各自推導或硬編碼
- [x] 驗證並修正回測中心開關到回測引擎的資料流；回測結果須提供重新入市啟用狀態、cycle 次數、同方向重新入市次序及可解釋事件證據
- [x] 驗證並修正參數快照的保存、預覽、套用、複製與舊版遷移；`reentryEnabled` 必須 round-trip 保真且舊快照明確預設關閉
- [x] 驗證並修正策略交易的一般新建、編輯及從參數快照建立／套用；策略卡必須顯示已保存狀態，不得由名稱或 UI 暫存值猜測
- [x] 在 KRM 策略卡加入精簡可觀測資訊：重新入市開關、目前方向、目前 cycle 內同方向入市次序與累計重新入市次數；無可信 runtime 資料時顯示「尚無紀錄」而非偽造 0
- [x] 【部分完成後依指示停止】建立回測指標唯一規格並修正所有計算／顯示：已完成核心績效 v2、回撤及 profit factor 等；其餘全欄位稽核未完成
- [x] 【部分完成後依指示停止】對每條已註冊策略建立機器可讀的回測準備度矩陣：僅完成 7/9 初步稽核，未建立正式守門矩陣
- [x] 【依指示跳過／未實作】在回測中心提供逐策略準備度、指標口徑、資料品質及不支援能力提示
- [x] 【部分完成後依指示停止】以獨立 oracle 與固定資料集補齊回歸測試：已完成核心績效與 KRM 重入案例，未完成全策略資料集
...
- [x] 依目前最新程式與驗證結果重新生成正式 Word 最終報告，納入事故根因、11,450.29 USDT 數值對帳、影響範圍、全鏈路修正、驗收矩陣與限制，完成 13／13 頁全頁視覺 QC 後作為附件交付
...
## 2026-08-05 Kama 彩虹馬丁回測破產診斷與修復（已分流，需另案確認；本輪未實施）

- [x] 已分流、未執行：診斷 KRM 三模式 funding 與 margin 計算缺陷；需另案確認後處理
- [x] 已分流、未執行：檢查馬丁倍率配置與初始資本匹配度；不得在未確認下自行改層數或資本
- [x] 已分流、未執行：驗證 55,000/55,488 根停止是否由 bankruptcy terminal 觸發
- [x] 已分流、未執行：修復 KRM 三模式 funding／margin；本輪沒有宣稱此問題已修復
- [x] 已分流、未執行：調整馬丁倍率或初始資本；屬交易與風控參數決策，需另案確認
- [x] 已分流、未執行：重新跑完整 55,488 根破產案例；本輪只驗證動態 KAMA 線路徑
- [x] 已分流、未執行：擴查 V50／V61／V70；本輪修改嚴格隔離於 KRM


## 2026-08-05 KRM 彩虹馬丁 MARGIN_USAGE_POLICY_BREACH 診斷（線數問題已完成；風控問題已分流）
- [x] Phase 1：已核讀並以 2／3／6／32 線測試驗證全部啟用 KAMA 共同參與入市分類
- [x] Phase 2：已確認馬丁／S1-M2-H3 屬另一條獨立風控工作流；本輪不修改其層級管理
- [x] Phase 3：未發現固定兩線的入市核心；已修復缺失 canonical 配置時的兩線 fallback 與可觀測性缺口
- [x] Phase 4：已完成六線回測、實盤配置傳遞及快照 round-trip 驗證；破產案例重跑另案處理

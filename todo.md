...
## 快照導入實盤倉位覆寫與七彩虹線區間馬丁重構（2026-07-28）

- [ ] 複核全端、排程與既有資料相容規範，盤點快照原始倉位、實盤部署倉位及下單數量完整資料流
- [ ] 定義所有現有與未來策略共用的「快照策略參數唯讀、實盤部署倉位可獨立覆寫」契約
- [ ] 修正從快照導入、建立副本、套用至現有策略等入口，使實盤倉位數值與單位可修改且不回寫快照
- [ ] 確保策略建立、更新、列表、執行器與下單數量正規化均優先使用實盤部署倉位覆寫值
- [ ] 將七彩虹線逐層馬丁模型重構為狀態、起層、終層、倍數、間距來源、自訂間距的連續區間模型
- [ ] 讓七彩虹線區間可新增、刪除、啟停並驗證無重疊、無缺口、起終層合法及無固定總層數上限
- [ ] 同步七彩虹線共享配置、正規化、驗證、回測器、實盤管理核心與加倉金額計算使用同一區間解析器
- [ ] 定義全局間距與自訂間距優先序，確保 UI 顯示的每一區間倍數及間距就是底層實際執行值
- [ ] 更新快照庫、回測快照、策略交易導入、匯入匯出與參數摘要為區間馬丁 canonical 格式
- [ ] 為既有逐層 Martin_Layers 快照提供無損轉換與 round-trip 相容，避免舊策略及歷史快照失效
- [ ] 新增通用實盤倉位覆寫、快照不可變、區間解析、舊格式升級、回測／實盤一致與策略隔離回歸測試
- [ ] 執行 TypeScript、相關 Vitest、全量 Vitest、生產建置及登入後 UI 視覺驗證
- [ ] 核對全部待辦、保存 checkpoint 並確認版本已自動發布

## 全策略訊號盈虧、V4.0 虧損交易調查與 API 帳號顯示（2026-07-28）

- [x] 以唯讀方式盤點 signals、trades／executions、strategies、api_keys 及交易所成交資料的關聯，不執行下單、平倉或策略狀態變更
- [x] 追查訊號日誌僅 V6.1 顯示盈虧及策略 `#60007` 缺少名稱的根因，涵蓋查詢、資料寫入、歷史相容與前端 fallback
- [x] 建立所有現有及未來策略共用的策略名稱解析契約，優先顯示實例名稱並為孤立歷史資料提供可追溯 fallback
- [x] 建立所有現有及未來策略共用的已實現盈虧記錄契約，區分開倉／加倉尚未實現與部分／全部平倉已實現盈虧
- [x] 盤點並統一自動平倉、手動平倉、部分平倉、止盈、止損、緊急全平、反向成交及各內建策略專用平倉路徑
- [x] 對 V4.0 KAMA+3K 動態馬丁策略三筆虧損平倉重建訂單、成交、費用、持倉方向、平倉原因與時間線，輸出逐筆證據報告
- [x] 核對平台盈虧與交易所已實現收益口徑，明確處理成交價差、手續費、部分平倉、合約面值與四捨五入
- [x] 擴充策略列表通用資料契約，為每張現有及未來策略卡片顯示所綁定 API 帳號的安全名稱、交易所及遮罩識別
- [x] 處理 API 金鑰已刪除、未綁定、跨帳號不可見與舊策略缺少關聯時的清楚 fallback，不暴露 API Key／Secret
- [x] 新增全策略名稱、平倉盈虧、歷史資料、V4.0 交易關聯與 API 帳號卡片回歸測試
- [x] 執行 TypeScript、定向 Vitest、全量 Vitest、生產建置及唯讀 UI 驗證；預覽截圖代理連續逾時，已以正式建置、HTTP 健康、前端契約與靜態檢查替代驗證，確認未觸發交易操作
- [x] 完成詳細根因與三筆交易分析報告，核對全部待辦、保存 checkpoint 並確認版本已自動發布

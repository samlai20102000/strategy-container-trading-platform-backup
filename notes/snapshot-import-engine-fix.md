...
- 已於 `tradeauto-ny5chipj.manus.space/strategies` 使用擁有者登入會話開啟「從快照導入」。
- V4.0 快照成功顯示 19 個完整原始參數，合法的 `0` 與 `false` 值沒有被內建預設覆蓋；倉位還原為 `100 USDT` 並鎖定單位。
- 策略引擎清楚顯示為 `20415_KAMA_MARTIN_V35`，介面標示「不可更換」，並說明伺服器會再次驗證，未註冊時停止建立而不回退其他引擎。
- 建立前再次確認部署帳戶為 `OKX模擬子帳號samlai02（OKX）`，快照倉位為 `100 USDT`、槓桿 `1x`，未操作任何實盤帳戶。
- 專用驗收策略名稱設為 `V4.0 快照導入驗收（未啟用自動）2026-07-25`，用以與既有運行中策略清楚區隔。
- 已成功建立上述策略；成功提示明確確認鎖定原引擎 `20415_KAMA_MARTIN_V35`。建立後卡片保留在 `Webhook` 模式，顯示可操作的「啟用自動」控制；驗收未點擊該控制。
- 生產策略頁現有 V6.1 與 V4.0 策略均顯示「自動」交易模式及可操作的「切回 Webhook」控制，證實不再因缺少 `strategyKey` 而令自動模式控制失效。
- 驗收期間沒有按下自動交易切換、手動觸發、測試信號、平倉或任何下單控制；本次驗收操作未觸發實盤或模擬訂單。
- **人工驗收結論：通用快照部署契約已在生產環境完整驗證通過。**

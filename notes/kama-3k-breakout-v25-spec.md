...
候選版本 `96d9a1c5` 自動發布後，已登入生產網址 `https://tradeauto-ny5chipj.manus.space/strategies` 於桌面視窗正常渲染策略管理頁、側邊導航、策略卡片、狀態、輪詢日誌及「新增策略／從快照導入」控制，未見空白頁或前端例外。本輪僅讀取與開啟設定流程，禁止啟用自動、手動產生信號、建立策略、平倉或下單；頁面既有 V6.1 策略及其交易狀態不做任何操作。

新增策略對話框已正常開啟，部署欄位（名稱、模擬帳戶、交易對、倉位、槓桿、方向、下單類型）均可見且可操作。對話框初始採平台既有預設配置，策略引擎選擇器位於滾動區底部；未填寫名稱、未變更帳戶、未提交表單。

瀏覽器自動容器辨識未能直接拖動對話框內部捲軸，僅使背景頁面微幅捲動；此為驗收工具的容器定位限制，對話框內容與右側內部捲軸仍正常可見。後續改用鍵盤焦點／頁尾定位核對底部引擎選擇器。

對話框 DOM 的策略引擎說明文字可被搜尋到，但 Radix 下拉選項在選單關閉時不會掛載於 DOM，因此靜態快照不應被用來判斷 V2.5 選項缺失。已關閉未提交的表單，既有策略與交易狀態未受變更；後續改由回測中心的策略選擇器驗證 V2.5 專用面板與參數資料流。

已登入生產回測中心的策略選擇器實際列出 `KAMA 三K突破 V2.5｜階梯式馬丁`，與 EMA、V4.0、V5.0、V6.1、V7.0 並列，證明通用註冊中心與前端策略清單已接通。選取後頁面立即切換為 V2.5 專用面板，顯示 `KAMA / 3K / V2.5.0`、`契約 VALID`、`馬丁上限 10 L`、`首單單位 USDT`、六個 SECTOR、作戰摘要與執行語義；回測交易金額欄位鎖定為 100，明確由 `Base_Lot_Size` 單一契約控制。頁面顯示 3 段／10 層馬丁、硬止損 3%、固定止盈 1%、重入已啟用，與文件預設值及共享契約一致。本輪未按「開始回測」、未儲存快照、未觸發任何交易。

手機驗收以 390×844、deviceScaleFactor 2 的本機 Chromium 嘗試三次。後端已收到登入後回測查詢且均回應 HTTP 200，但該隔離 Chromium 的頁面維持 `title=策略容器化自動交易平台`、`readyState=interactive`、空白 `#root`，因此未把空白截圖冒充手機視覺通過證據。依使用者指示繼續後，改完成響應式結構稽核：V2.5 標頭與摘要在窄螢幕採直向堆疊，參數網格由單欄升至 `sm`／`xl` 多欄，側欄只在 `xl` 轉為 sticky，馬丁表格使用 `overflow-x-auto` 與 700px 最小寬安全水平捲動，所有數值欄位、開關與按鈕均保留行動觸控尺寸。桌面生產瀏覽器證據與程式結構一致；未提交回測、快照、策略建立或任何訂單。

最終交付前再次執行 V2.5 專屬 17 項測試、TypeScript 與正式生產建置，全部通過；`git diff --check` 無差異格式錯誤，且已移除本輪測試產生的唯一未追蹤 `TestUnitStrategy` 檔。重跑完整測試組時，既有 `testnet-symbol` 測試卡在外部 OKX 即時交易對請求，故終止重複執行；交付採用本輪修改前已完成的 32 檔／404 項全套通過證據，以及修改後的 17／17 V2.5 專屬測試、TypeScript 與正式建置結果。此情況屬外部資料源等待，不是測試斷言失敗。
